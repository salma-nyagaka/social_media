...
from social_media_project.apps.notification_service.tasks import send_batch_notifications, send_bulk_emails
...
class TestNotificationTasks:
    ...
    def setup_method(self, db):
        ...
        self.users = [
            User.objects.create(username=f'user{i}', email=f'user{i}@example.com', is_active=True)
            for i in range(5)
        ]
    ...
    def test_send_batch_notifications(self):
        ...
        result = send_batch_notifications(subject, message, self.user_emails, self.context, html_template, notification_type)
        ...
        for email in mail.outbox:
            ...
            assert html2text.html2text(email.alternatives[0][0]).strip() == expected_text
    ...
    def test_send_bulk_emails(self):
        ...
        # Check the content of each email
        for email, (_, text_content, _, _, html_content) in zip(mail.outbox, email_messages):
