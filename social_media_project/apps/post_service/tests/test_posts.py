...
class TestPostAPI(TestCase):
    ...
    def test_create_post(self, mock_send_email_task):
        ...
        """
        url = reverse('post-list')  # Update this to the correct name for your post list view
        data = {
            'title': 'New Post',
    ...
    def test_create_comment(self, mock_send_email_task):
        ...
        """
        url = reverse('comment-list')  # Update this to the correct name for your comment list view
        data = {
            'post': self.post.id,
        ...
        mock_send_email_task.assert_called_once()
        call_args = mock_send_email_task.call_args[1]
        assert call_args['subject'] == 'New comment has been added to the post: Test Post'
